...
A port of ZeroTier to the Nintendo Switch as a sysmodule, very AI assisted in the making, yet extremely functional with better results than I had hoped. Get the latest release from the releases page, or read the build instructions in build.md
...
if they all work... which brings me to the next point.
 ...
 There is a Ultrahand overlay ui. It shows the current status, lets a
  ...
  NIFM and detailed logging, these logs get placed in `/config/sys-GRID0` and 
  ...
  myself.)
   ...
   Saved networks live in `/config/sys-GRID0/networks.ini`, so an update does not
  wipe them out.
...
need an ldn MITM before they can use GRID0. The LAN whitelist covers the
